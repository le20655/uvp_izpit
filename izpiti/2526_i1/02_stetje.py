...
# Štetje
#
# Za pomoč pri štetju elementov bomo sestavili razred `Stevec`. Ta bo hranil število ponovitev vsakega elementa neke zbirke elementov.
# =====================================================================@044777=
# 1. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        ...
        try:
            tests = [
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko(1)", 2),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko(2)", 2),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko(3)", 2),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko(4)", 1),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko(5)", 1),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]).koliko('a')", 0),
            ]
    ...
    if Check.part():
        ...
        try:
            tests = [
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]) == Stevec([5, 4, 3, 3, 2, 2, 1])", False),
                ("Stevec([1, 2, 3, 4, 5, 1, 2, 3]) == Stevec([5, 4, 3, 3, 2, 2, 1, 1])", True),
            ]
            ...
            Check.run(test_dodaj, {'enako': True}, env={'Stevec': Stevec})
            ...
            Check.run(test_dodaj2, {'prvo': 0, 'drugo': 3}, env={'Stevec': Stevec})
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
    ...
    if Check.part():
        ...
        try:
            ...
            Check.run(test_unija, {'prvo': 2, 'drugo': 3, 'tretje': 4}, env={'Stevec': Stevec})
            test_unija2 = [
                "s = Stevec([1, 2, 3, 4, 5, 1, 2, 3])",
                "u = Stevec(['a', 'b', 'c', 'a', 'a'])",
                "z = s.unija(u)",
            ...
            Check.run(test_unija2, {'prvo': 1, 'drugo': 3}, env={'Stevec': Stevec})
            ...
            Check.run(test_presek, {'prvo': 0, 'drugo': 0}, env={'Stevec': Stevec})
            ...
            Check.run(test_presek2, {'prvo': 1, 'drugo': 1, 'tretje': 2}, env={'Stevec': Stevec})
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
