...
# Značke
#
# Raziskati želimo uporabo značk (angl. _hashtags_) v objavah na družbenem omrežju. Značka se prične z znakom `#` in nadaljuje z enim ali več alfanumeričnimi znaki (črke in števke), npr. `#python` ali `#easy123`. Pred in za značko je lahko presledek, ni pa to nujno.
# 
# Objave so definirane s seznamom parov `(oseba, vsebina)`, npr.:
# 
#     objave = [
#         ("Ana", "hello world #hello"),
#         ("Cene", "hello world"),
#         ("Bojan", "pozdravljen svet #hello #world"),
#         ("Bojan", "pozdravljen #svet"),
#         ("Cene", "pišem izpit #uvp #python #10"),
#         ("Ana", "to ni značka: #"),
#     ]
# =====================================================================@044774=
# 1. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        ...
        try:
            Check.equal('znacke("hello world #hello")', {'#hello'})
            Check.equal('znacke("###")', {})
            Check.equal('znacke("#a #b #c #b")', {'#a', '#b', '#c'})
            Check.equal('znacke("pozdravljen svet")', {})
            Check.equal('znacke("#easy #123")', {'#easy', '#123'})
            Check.equal('znacke("#a#b#c")', {'#a', '#b', '#c'})
        except TimeoutError:
    ...
    if Check.part():
        ...
        try:
            ...
            Check.equal(f'delez_objav_z_znackami({objave1}, "Ana")', 1/2)
            Check.equal(f'delez_objav_z_znackami({objave1}, "Bojan")', 1)
            Check.equal(f'delez_objav_z_znackami({objave1}, "Cene")', 1/2)
            Check.equal(f'delez_objav_z_znackami({objave2}, "Ana")', 0)
            Check.equal(f'delez_objav_z_znackami({objave2}, "Bojan")', 2/3)
            Check.equal(f'delez_objav_z_znackami({objave2}, "Cene")', 1)
            Check.equal(f'delez_objav_z_znackami({objave2}, "Denis")', None)
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
    ...
    if Check.part():
        ...
        try:
            Check.equal(f'najbolj_znackast({objave1})', 'Bojan')
            Check.equal(f'najbolj_znackast({objave2})', 'Cene')
        except TimeoutError:
            Check.error("Dovoljen čas izvajanja presežen")
        except Exception:
            Check.error(
